...
```{r, echo=T}
var_ls = VAR(dt, lags = lags, method = "ols")
var_mle = VAR(dt, lags = lags, method = "mle")
```
...
```{r, echo=T}
invisible(VAR_stable(VAR(dt, lag=3))$test_result)
```
...
varresult <- VAR(dt[,.SD,.SDcols=var_cols], lags = lags)
print(varresult)
```
...
```{r, fig.dim=c(7,4)}
fitted_values <- predict(varresult)
plot(fitted_values, y_true=varresult$y_train)
```
