...
DT::datatable(dt) %>%
  DT::formatRound(columns = 1:ncol(dt))
```
...
```{r, echo=T}
var_ls = VAR(dt[,.SD,.SDcols=var_cols], lag = lags, method = "ls", verbose = F)
var_mle = VAR(dt[,.SD,.SDcols=var_cols], lag = lags, method = "mle", verbose = F)
```
...
Standard univariate tests (e.g. ADF) can be used to test individual series for non-stationarity. For simplicity, let us take first differences of all series, which upon visual inspection look stationary pre. 
...
```{r, echo=T}
invisible(VAR_stable(VAR(dt[,.SD,.SDcols=var_cols], lag=3, verbose = F))$test_result)
```
...
varresult = VAR(dt[,.SD,.SDcols=var_cols], lag = lag, method = "ls", verbose = F)
DT::datatable(varresult$coeff_tidy) %>%
  DT::formatRound(columns = 1:ncol(var_ls$coeff_tidy))
```
...
```{r}
n.ahead = 10
fig_cap = sprintf("%i-step ahead predictions from estimated VAR.",
                  n.ahead)
```
...
In R, we can predict from a model that has been estimated through `VAR` using the `VAR_predict` function. The figure below shows the `r n.ahead`-step ahead predictions for our model. 

```{r predict, fig.dim=c(7,4), fig.cap=fig_cap}
my_pred = VAR_predict(varresult, n.ahead, plot = T, theme = theme_bw())
```
