...
library(deepvars)
data("canada")
splits_ <- train_val_test_split(canada[,-1])
...
y_hat <- data.frame(Reduce(cbind,test_preds))
colnames(y_hat) <- colnames(canada)[-1]
y_hat <- t(t(y_hat) * train_ds$train_sd + train_ds$train_mean)
y_hat <- melt(data.table(y_hat)[,id:=1:.N], id.vars = "id", value.name = "y_hat")
setkey(y_hat, id, variable)

library(data.table)
y_true <- melt(data.table(df_test[(lags+1):(nrow(df_test)-n_ahead),])[,id:=1:.N], id.vars="id",value.name="y")
setkey(y_true, id, variable)

dt_plot <- melt(y_hat[y_true],id.vars = c("id","variable"), variable.name = "type")
library(ggplot2)

ggplot(dt_plot, aes(x=id, y=value, colour=type)) +
  geom_line() +
  geom_point() +
